...
//  Marconi
//
//  Created by Shane Whitehead on 3/12/18.
...
public class BannerController: NSObject {
    ...
    fileprivate var contentOffsetObserver: NSKeyValueObservation?
    ...
    public func install(on scrollView: UIScrollView) {
        ...
        contentOffsetObserver = scrollView.observe(\.contentOffset) { [weak self] scrollView, change in
            ...
            observableStateDidChangeFor(scrollView: scrollView)
        }
    ...
    private func observableStateDidChangeFor(scrollView: UIScrollView) {
        guard scrollView.isUserInteractionEnabled && !scrollView.isHidden else { return }
